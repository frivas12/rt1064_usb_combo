...
#define RT_UPDATE_ADDR_MIN 0x60000000UL
#define RT_UPDATE_ADDR_MAX 0x60800000UL
...
{
    (void)memset(s_rt_update.page_buffer, 0xFF, sizeof(s_rt_update.page_buffer));
}
...
static bool rt_update_spi_exchange(const uint8_t *tx, uint8_t *rx, uint16_t length)
{
    if (length == 0U)
    {
        return true;
...
static bool rt_update_enter_update_mode(void)
{
    ...
    {
        if (rt_update_send_command(RT_UPDATE_CMD_ENTER_UPDATE_MODE, NULL, 0U, NULL, 0U))
        {
            return true;
        }
    }
...
static bool rt_update_erase_sector(uint32_t base_address)
{
    ...
    for (uint8_t attempt = 0; attempt < RT_UPDATE_RETRY_MAX; ++attempt)
    {
        if (rt_update_send_command(RT_UPDATE_CMD_ERASE_SECTOR, payload, sizeof(payload), NULL, 0U))
        {
            return true;
        }
    }
...
static bool rt_update_program_page(uint32_t base_address, const uint8_t *data)
{
    ...
    for (uint8_t attempt = 0; attempt < RT_UPDATE_RETRY_MAX; ++attempt)
    {
        if (rt_update_send_command(RT_UPDATE_CMD_PROGRAM_PAGE, payload, sizeof(payload), NULL, 0U))
        {
            return true;
        }
    }
...
static bool rt_update_read_back(uint32_t base_address, uint8_t *buffer)
{
    ...
    for (uint8_t attempt = 0; attempt < RT_UPDATE_RETRY_MAX; ++attempt)
    {
        if (!rt_update_send_command(RT_UPDATE_CMD_READ_BACK, payload, sizeof(payload),
...
static bool rt_update_flush_page(USB_Slave_Message *slave_message)
{
    ...
    if ((!s_rt_update.sector_valid) || (sector_base != s_rt_update.current_sector_base))
    {
    ...
    if (!rt_update_program_page(s_rt_update.current_page_base, s_rt_update.page_buffer))
    {
        return false;
    }
...
static bool rt_update_handle_data(uint32_t address, const uint8_t *data, uint8_t length,
...
static bool rt_update_finalize(USB_Slave_Message *slave_message)
{
    ...
    if (!rt_update_send_command(RT_UPDATE_CMD_FINALIZE_AND_REBOOT, NULL, 0U, NULL, 0U))
    {
        return false;
    }
                                   ...
                                   USB_Slave_Message *slave_message)
{
    ...
    if (!rt_hex_parse_line(line, length, data, sizeof(data),
                           &data_len, &offset, &record_type))
    {
    ...
    case 0x00:
    {
        ...
        return rt_update_handle_data(address, data, data_len, slave_message);
    }
    case 0x01:
