...
!include "x64.nsh"
!define MAJOR_VERSION ${PRODUCT_VERSION}
!define APP_NAME ${PRODUCT_NAME}
!define PRODUCT_PUBLISHER "Thorlabs"
!define PRODUCT_WEB_SITE "https://www.thorlabs.com"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\${EXECUTABLE}"
...
OutFile "${OUT_DIR}\${APP_NAME}-Installer.exe"
...
Section "Application" SEC01
  ...
  ; ${If} ${RunningX64}
  ;   ExecWait "$INSTDIR\Driver\dpinst64.exe /sa /p"
  ; ${Else}
  ; ${EndIf}
  ...
  ;Install drivers
  IfFileExists "$INSTDIR\Driver\*.*" 0 +5
  DetailPrint "Found driver folder"
  ${DisableX64FSRedirection}
  nsExec::ExecToLog '"$SYSDIR\PnPutil.exe" /a "$INSTDIR\Driver\*.inf"'
...
Section -AdditionalIcons
  ...
  CreateShortCut "$SMPROGRAMS\Thorlabs ${PRODUCT_FOLDER_NAME}\${PRODUCT_NAME} ${MAJOR_VERSION}.lnk" "$INSTDIR\${EXECUTABLE}"
  CreateShortCut "$DESKTOP\${PRODUCT_NAME} ${MAJOR_VERSION}.lnk" "$INSTDIR\${EXECUTABLE}"
...
Section -Post
  SetOutPath $INSTDIR
  WriteUninstaller "$INSTDIR\uninst_${PRODUCT_NAME}_${MAJOR_VERSION}.exe"
  ...
  WriteRegStr HKLM "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\${EXECUTABLE}"
