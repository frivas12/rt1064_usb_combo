 ...
 *  *@dot
 ...
 node [shape=ellipse];
 "init clocks and irq" -> "enable caches" -> "disable TCM" -> "start peripherals";
 "start peripherals" -> "reset CPLD" -> "setup SPI" -> "config USB Host";
 "config USB Host" -> "initialize slots" -> "config error thread" -> "return";
 "config USB Host" -> "usb host task";
 "config error thread" -> "error task";
 }
 * @enddot
 */
#include <asf.h>
...
static void program_fuses(void)
{
	...
	locked_regions = flash_is_locked(BOOTLOADER_FIRST_SECTOR, BOOTLOADER_LAST_SECTOR);
...
static void use_mainclk_before_sysclk_init(void)
{
#if (CONFIG_SYSCLK_SOURCE == SYSCLK_SRC_PLLACK || CONFIG_SYSCLK_SOURCE == SYSCLK_SRC_UPLLCK)
...
/**
 * @brief Initialize the processor details, the peripherals, then board and slot details
 *
 */
...
//	WDT->WDT_MR = WDT_MR_WDDIS | WDT_MR_WDDBGHLT;
	...
	// Initialize slot synchronization objects to prevent system hang (even when programming cpld).
	...
	/*Only continue if the CPLD has been programmed, else the usb slave is running to load the
	 * CPLD with software*/
