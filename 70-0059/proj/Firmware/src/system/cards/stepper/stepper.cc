...
static bool parse_active(Stepper_info *info, USB_Slave_Message *slave_message,
    ...
    case MGMSG_MOT_MOVE_STOP: /* 0x0465*/
        if (slave_message->param2 == 0x00) {
            slave_message->param2 = Tst_bits(info->save.config.params.flags.flags, PREFERS_SOFT_STOP)
                    ? 0x02 : 0x01;
        }

        switch(slave_message->param2) {
        case 0x01:
            info->ctrl.mode = STOP;
            *p_log_id = STEPPER_STOP;
            break;
        case 0x02:
            info->ctrl.mode = SOFT_STOP_START;
            *p_log_id = STEPPER_STOP;
            break;
        default:
            break;
