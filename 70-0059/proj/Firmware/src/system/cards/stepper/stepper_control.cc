...
void service_stepper(Stepper_info *info, USB_Slave_Message *slave_message)
{
	...
	case SOFT_STOP_START:
		soft_stop_stepper(info->slot);
        info->ctrl.mode = SOFT_STOP_WAIT;

		info->ctrl.cur_vel = 0;
		info->ctrl.homing_mode = HOME_IDLE;
		info->limits.hard_stop = false;

        log_id = STEPPER_CONTROLLER_SOFT_STOP_ISSUED;
        break;

    case SOFT_STOP_WAIT:
        if (Tst_bits(info->ctrl.status_bits, STATUS_CCW_MOVE) || Tst_bits(info->ctrl.status_bits, STATUS_CW_MOVE)) {
            break;
        }
		...
		else
		{
			...
			info->ctrl.mode = IDLE;
			log_id = STEPPER_CONTROLLER_SOFT_STOP_COMPLETED;
		}
		info->counter.cmnd_pos = info->enc.enc_pos;
        break;
