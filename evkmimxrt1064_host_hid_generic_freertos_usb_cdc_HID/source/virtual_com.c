 ...
 * Definitions
 ******************************************************************************/
#ifndef CDC_APP_TASK_STACK_SIZE
#define CDC_APP_TASK_STACK_SIZE 5000L
#endif
...
void USB_DeviceCdcVcomIsr(void);
...
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE) static uint8_t s_currRecvBuf[CDC_DATA_BUFF_SIZE];
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE) static uint8_t s_currSendBuf[CDC_DATA_BUFF_SIZE];
...
usb_status_t USB_DeviceCallback(usb_device_handle handle, uint32_t event, void *param)
{
    ...
    (defined(USB_DEVICE_CONFIG_LPCIP3511HS) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
            ...
            if (kStatus_USB_Success == USB_DeviceClassGetSpeed(CDC_CONTROLLER_ID, &s_cdcVcom.speed))
            {
 ...
 * @return None.
 */
static void USB_DeviceCdcVcomApplicationInit(void)
{
    USB_DeviceClockInit();
#if (defined(FSL_FEATURE_SOC_SYSMPU_COUNT) && (FSL_FEATURE_SOC_SYSMPU_COUNT > 0U))
 ...
 */
#if USB_DEVICE_CONFIG_USE_TASK
void USB_DeviceCdcVcomTask(void *handle)
{
    while (1U)
    {
 ...
 * @return None.
 */
static void USB_DeviceCdcVcomAppTask(void *handle)
{
    usb_status_t error = kStatus_USB_Error;
    uint32_t usbOsaCurrentSr;

    USB_DeviceCdcVcomApplicationInit();

#if USB_DEVICE_CONFIG_USE_TASK
    if (s_cdcVcom.deviceHandle)
    {
        if (xTaskCreate(USB_DeviceCdcVcomTask,           /* pointer to the task                      */
                        (char const *)"usb device task", /* task name for kernel awareness debugging */
                        5000L / sizeof(portSTACK_TYPE),  /* task stack size                          */
                        s_cdcVcom.deviceHandle,          /* optional task startup argument           */
                        CDC_DEVICE_USB_TASK_PRIORITY,    /* initial priority                         */
...
usb_status_t USB_DeviceCdcVcomInit(void)
{
    if (xTaskCreate(USB_DeviceCdcVcomAppTask,                         /* pointer to the task                      */
                    s_appName,                                        /* task name for kernel awareness debugging */
                    CDC_APP_TASK_STACK_SIZE / sizeof(portSTACK_TYPE), /* task stack size                          */
                    &s_cdcVcom,                                       /* optional task startup argument           */
                    CDC_DEVICE_APP_TASK_PRIORITY,                     /* initial priority                         */
                    &s_cdcVcom.applicationTaskHandle                  /* optional task handle to create           */
                    ) != pdPASS)
    {
        usb_echo("cdc vcom app task create failed!\r\n");
